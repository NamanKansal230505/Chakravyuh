...
/* Includes ---------------------------------------------------------------- */
#include <Chakravyuh2_inferencing.h>
#include <WiFi.h>
...
#define WIFI_PASSWORD "9456943553@"
...
void loop()
{
    ...
    for (size_t ix = 0; ix < EI_CLASSIFIER_LABEL_COUNT; ix++) {
        ...
        if(result.classification[ix].value>largest){
          ...
          lab = result.classification[ix].label;
        }
    }
    if(largest>0.97){
      Serial.println(lab);
      Serial.println(largest);      Firebase.RTDB.setBool(&fbdo, "/nodes/node1/alerts/"+lab, true);
    }
